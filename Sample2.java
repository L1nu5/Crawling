...
public class Sample2{
    ...
    int numberOfPages = 67; // first two pages of results will be fetched
    ...
    try 
    {
        ...
        for (int i = 0; i < numberOfPages; i++) {
            // query reviews
            Response response = Jsoup.connect(reviewApiCall+(i*15)).userAgent(userAgent).referrer(referer).timeout(100000).header("x-user-agent", xUserAgent).header("host", host).ignoreContentType(true).execute();
        ...
        // parse json response
            ...
            for (Object object : jsonArray) {
                ...
                System.out.println("Author: " + jsonObject.get("author") + "\thelpful: "+ jsonObject.get("helpfulCount") + "\n\t"+ jsonObject.get("text").toString().replace("\n", "\n\t") + "\n");
            }
        }
    } catch (Exception e) {
        e.printStackTrace();
